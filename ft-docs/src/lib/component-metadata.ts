...
function findStoryFile(componentName: string): string | null {
  ...
  const componentStoryPaths = [
    path.join(srcComponentsDir, 'atoms', componentName, `${componentName}.stories.tsx`),
    path.join(srcComponentsDir, 'molecules', componentName, `${componentName}.stories.tsx`),
    path.join(srcComponentsDir, 'organisms', componentName, `${componentName}.stories.tsx`),
  ];
...
function extractExamplesFromStory(storyPath: string, componentName: string): Array<{ name: string; code: string }> {
  ...
  const storyExports = Array.from(content.matchAll(/export\s+const\s+(\w+)\s*:\s*Story\s*=\s*\{/g));

  // Separate Figma stories from regular stories
  ...
  for (const match of storyExports) {
    ...
    if (/Figma|figma/i.test(storyName)) {
      figmaStories.push(match);
    } else {
      // Include ALL regular stories - we'll filter based on extracted props later
      // Don't skip Default - it might be needed for components without other variants
      regularStories.push(match);
    }
  }

  // Process regular stories first, then Figma stories only if no regular stories
  // This ensures individual variants are shown, and FigmaVariants only if no individual variants exist
  const storiesToProcess = regularStories.length > 0 ? regularStories : figmaStories;
