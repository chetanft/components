...
export interface DataEntryTableCellAmountProps {
  value?: string | number;
  placeholder?: string;
  error?: string;
...
export const DataEntryTableCellAmount: React.FC<DataEntryTableCellAmountProps> = ({
  value,
  placeholder = 'Value',
  error,
  state = 'default',
  ...
  return (
    ...
    >
      ...
      >
        {currencySymbol && (
          <span className="overflow-ellipsis overflow-hidden relative shrink-0"
          style={{
            fontSize: 'var(--font-size-md)',
            color: hasValue ? 'var(--primary)' : 'var(--tertiary)',
          }}
        ...
        {effectiveState === 'error-filled' && (
          <Icon
            ...
            size={16}
            className="text-[var(--critical)] shrink-0 ml-[var(--spacing-x1)]"
          />
        )}
      </div>
    </div>
...
DataEntryTableCellAmount.displayName = 'DataEntryTableCellAmount';
