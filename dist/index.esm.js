...
const Chicklet = forwardRef(({ label, variant = 'rectangular', showClose = true, onClose, onClick, disabled = false, className, ...props }, ref) => {
...
const DatePickerField = forwardRef(({ size, state, type, value, placeholder = "Value", disabled, showTime, className, onChange, onFocus, onBlur, ...props }, ref) => {
...
const DatePicker = forwardRef(({ label, labelPosition = "top", required, size = "xl", showTime, className, ...fieldProps }, ref) => {
    const renderLabel = () => {
...
const DropdownField = forwardRef(({ size, state, type, value, placeholder = "Value", disabled, open, className, onClick, onFocus, onBlur, ...props }, ref) => {
    const iconColor = state === "disabled" ? "text-gray-400" : "text-gray-800";
...
const Dropdown = forwardRef(({ label, labelPosition = "top", required, size = "xl", caption = "This is caption under text", showCaption = false, className, options = [], onSelect, ...fieldProps }, ref) => {
...
const TabItem = forwardRef(({ label, badge = false, badgeCount = "56", notification = false, icon = false, active = false, type = 'primary', onSelect, className, ...props }, ref) => {
    const [isHovered, setIsHovered] = useState(false);
    // Get current state
...
const Tabs = forwardRef(({ showLine = true, tabs, activeTab = 0, onTabChange, type = 'primary', className, ...props }, ref) => {
...
const TableRowComponent = ({ row, columns, index, variant = 'primary', selectable, selected = false, onSelectionChange }) => {
    ...
    const renderCellContent = useCallback((column, value) => {
        if (column.render) {
...
const Table = ({ columns, data, variant = 'primary', selectable = false, selectedRows = [], onSelectionChange, onSort, sortColumn, sortDirection, pagination, loading = false, emptyMessage, className }) => {
    ...
    const handleRowSelectionChange = useCallback((rowId, selected) => {
        if (!onSelectionChange)
            return;
        if (selected) {
